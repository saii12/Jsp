...
public class ProductDAO extends DBHelper{
	...
	public ProductDTO selectProduct(String pNo) {
		...
		try {
			conn = getConnection();
			psmt = conn.prepareStatement(SQL.SELECT_PRODUCT_);
			psmt.setString(1, pNo);
			rs = psmt.executeQuery();
			
			if(rs.next()) {
				dto.setpNo(rs.getInt(1)); //칼럼값 모두 속성으로 일단 지정해주자
	...
	// 추가
	public int selectCountProductsTotal(String type) { //리턴값 없을 땐 void
		int total = 0;
