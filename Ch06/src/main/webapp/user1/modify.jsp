...
<%
	...
	try {
		...
		ResultSet rs = psmt.executeQuery(); //데이터베이스에서 조회한 값을 rs객체로 저장
		
		if(rs.next()){ 
			vo.setUid(rs.getString(1)); //데이터베이스에서 조회한 값을 vo객체의 속성에 저장
	...
	<body>
		...
		<form action="/Ch06/user1/modifyProc.jsp" method="post"> 
			<table border="1">
				<tr>
					<td>아이디</td>
					<td><input type="text" name="uid" readonly value="<%= vo.getUid() %>"></td> <!-- readonly 입력 못하고 읽기만 가능(primary key이므로) -->
				</tr>
