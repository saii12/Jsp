...
$(function(){
	...
	$('#btnEmailCode').click(function(){
		...
		$('.resultEmail').text('인증코드 전송중입니다. 잠시만 기다리세요...');
		$('.resultEmailForId').text('인증코드 전송중입니다. 잠시만 기다리세요...');
		$('.resultEmailForPass').text('인증코드 전송중입니다. 잠시만 기다리세요...');
				...
				success: function(data){
					...
					if(data.result > 0){
						...
						if(data.status > 0){
							...
							$('input[name=auth]').prop('disabled', false); // 전송 성공할 때 인증번호창 활성화
						}else{
							$('.resultEmailForId').css('color', 'red').text('인증코드 전송 실패했습니다. 잠시 후 다시 시도하십시오.');
							$('.resultEmailForPass').css('color', 'red').text('인증코드 전송 실패했습니다. 잠시 후 다시 시도하십시오.');
						}
						...
						if(data.status > 0){
							...
							$('.resultEmail').css('color', 'red').text('인증코드 전송 실패했습니다. 잠시 후 다시 시도하십시오.');
